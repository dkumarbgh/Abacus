...
async function migrate() {
    ...
    for (const [name, def] of [
        ...
        // Drives auto-generated per-student Fee Plan periods (see
        // services/feePlanGenerator.js) - when a student's fee periods
        // should start counting from, so someone joining in August on a
        // Monthly plan owes from August onward, not from the start of
        // the academic year.
        ["date_of_joining", "TEXT"],

        // Which Fee Plan (if any) this student was explicitly assigned to
        // at admission - chosen from the Add/Edit Student form, since a
        // Level can offer more than one plan (e.g. Monthly vs Quarterly)
        // and different families may want different frequencies. NULL
        // means no plan chosen (fees are handled manually, or via a
        // level-wide Fee Structure item instead).
    ...
    // Monthly installment support: when a fee (e.g. "Annual Fees") is
    // split into months rather than one lump amount, each month becomes
    // its own fee_structure row - same fee_category_id, level_id, and
    // academic_year as the others in that breakdown, but its own
    // (independently editable) amount and this label ("April 2026" etc)
    // to tell them apart on screen. NULL means "not part of an
    // installment breakdown" - a normal single-amount fee, same as
    // every fee_structure row before this column existed.
    await addColumnIfMissing("fee_structure", "installment_label", "TEXT");

    // Payment Mode: how a fee is meant to be split for tracking/followup -
    // 'single' | 'monthly' | 'bimonthly' | 'quarterly' | 'half_yearly'.
    // Every row belonging to the same installment plan shares the same
    // mode, so reports can group/filter "all Quarterly fees" etc.
    // Existing rows (created before this column existed) default to
    // 'single', matching how they already behaved - one lump amount.
    await addColumnIfMissing("fee_structure", "payment_mode", "TEXT NOT NULL DEFAULT 'single'");

    // Fee Plans: a per-Level "rate" (e.g. "Level 1, Monthly, ₹1000/period")
    // rather than a pre-generated calendar template. When a student joins
    // (or is assigned to a Level that has an active plan), their OWN
    // fee_structure rows get generated starting from THEIR join date, not
    // a fixed academic-year start - see services/feePlanGenerator.js.
    await run(`
    ...
    // Explicit end date for a Fee Plan's period generation, replacing
    // reliance on parsing academic_year into a fixed 31-March cutoff -
    // lets an Admin set exactly how far out fees are scheduled (e.g. one
    // year from when the plan is set up, the UI's default) rather than
    // always running to a fixed academic-year boundary. Old plans created
    // before this column existed fall back to the academic-year-based
    // cutoff - see services/feePlanGenerator.js.
    await addColumnIfMissing("fee_plans", "end_date", "TEXT");

    // Superseded end_date above: a shared calendar cutoff meant every
    // student's LAST period landed on the same date regardless of when
    // they joined, which isn't what "one year of fees" should mean for
    // a student who joins mid-year. duration_months instead gives EVERY
    // student the same LENGTH of schedule (default 12 months), counted
    // from their own Date of Joining - joining in July means periods
    // run July through the following June, not to a shared date. Falls
    // back to end_date/academic_year only for plans created before this
    // column existed - see services/feePlanGenerator.js.
    await addColumnIfMissing("fee_plans", "duration_months", "INTEGER NOT NULL DEFAULT 12");

    // Tags a fee_structure row as "generated by this Fee Plan" - lets
    // generation be safely re-run (e.g. "assign to students who joined
    // since this plan was created") without ever duplicating a period
    ...
    // Per-school custom text for any UI label (see services/labels.js) -
    // lets a school override a label's wording without touching code,
    // e.g. renaming "Batch" to "Section". One row per (school, key)
    // overridden; a key with no row here just uses the language default.
    await run(`
        CREATE TABLE IF NOT EXISTS label_overrides(
            ...
            FOREIGN KEY(school_id) REFERENCES schools(id),
            UNIQUE(school_id, label_key)
        )
    `);

    // Audit Log: a record of significant changes (create/update/delete)
    // across the app, so an Admin can see who changed what and when.
    // branch_id is filled in whenever the change relates to a specific
    // student (derived from that student's own branch_id at the time),
    // since Branch is fundamentally a per-student attribute in this app -
    // it's left NULL for changes that aren't tied to any one student
    // (e.g. user account changes), which still show up in the log but
    // won't match a specific Branch filter.
    // Scope note: this covers the highest-value actions (Students, Fee
    // Payments/Discounts/Structure, Attendance, Exams, User accounts) -
    // not literally every mutation in the app. See services/auditLog.js
    // to add logging to further actions.
    await run(`
    ...
    // Language + per-school label customization (see services/labels.js).
    // 'en' or 'kn' (Kannada) for now - more languages just need adding to
    // the DEFAULTS dictionary there, nothing here needs to change.
    await addColumnIfMissing("schools", "language", "TEXT NOT NULL DEFAULT 'en'");

    // Per-school Import/Export feature toggles for Attendance and Fee
    // Payments (separate from the student one, which is a deployment-wide
    // env flag in config/features.js - these are per-school, changeable
    // from Settings without a redeploy, since different schools may want
    // this on/off independently).
    ...
    // Collection, Fee Collection Summary, Student Payment History) and
    // can be discounted/waived per student the exact same way any other
    // fee already can, with zero new fee-tracking code needed.
    await addColumnIfMissing("exams", "fee_structure_id", "INTEGER REFERENCES fee_structure(id)");
