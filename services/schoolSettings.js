...
const FIELD_DEFS = {
    student: {
        admission_no:   { label: "Roll Number",           defaultMandatory: false },
        ...
        guardian_phone: { label: "WhatsApp 1 Number", defaultMandatory: true },
...
/**
 * Fetches the school's configured default "hours attended" value, used to
 * pre-fill the hours box in Attendance Management. Returns a Promise<number>.
 * Falls back to 2 if the school row can't be found (should not normally
 * happen since the column has a DB-level default of 2 already).
 */
function getDefaultHoursAttended(schoolId) {
    return new Promise((resolve, reject) => {
        db.get("SELECT default_hours_attended FROM schools WHERE id=?", [schoolId], (err, row) => {
            if (err) return reject(err);
            resolve(row && row.default_hours_attended != null ? row.default_hours_attended : 2);
        });
    });
}
