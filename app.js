...
["students", "tmp", "sheets", "faces", "certificates", "test-papers"].forEach(dir => {
...
const levelPromotionRoutes = require("./routes/levelPromotion");
const testPaperRoutes = require("./routes/testPapers");
...
app.use("/level-promotion", levelPromotionRoutes);
app.use("/test-papers", testPaperRoutes);
                        ...
                        (err, teacherCount) => {
                            ...
                            db.all(
                                `SELECT students.name,
                                 ...
                                 ON students.level_id = level.id AND level.list_type = 'level'
                                ...
                                (err, recentStudents) => {
                                    ...
                                    // Grouped by Level rather than Class - a batch can mix
                                    // students from several levels, so Level is the more
                                    // meaningful breakdown here (matches the rest of the app,
                                    // where Level is the primary grouping and Class stays
                                    // in the background).
                                    db.all(
                                        `SELECT level.name AS level_name,
                                                COUNT(students.id) total
                                         FROM lookup_items level
                                         LEFT JOIN students
                                         ON students.level_id = level.id AND students.school_id = ?
                                         WHERE level.school_id = ? AND level.list_type = 'level'
                                         GROUP BY level.id
                                         ORDER BY level.name`,
                                        [schoolId, schoolId],
                                        (err, levelSummary) => {
